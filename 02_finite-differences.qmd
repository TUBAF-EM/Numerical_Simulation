...
Assume the general Poisson equation
$$-\div(a\grad u)=f$$

with some conductivity $a$, a source term $f$ and the potential $u$.
This can be an electric potential with $a=\sigma$, the hydraulic head with hydraulic conductivity $k$, the temperature with the thermal conductivity $\lambda$ or the gravity potential with $a=1$.
...
## FD Approximation in 1D

The basic idea of the Finite Difference method is to approximate the partial derivatives by finite derivatives using a predefined discretization.

:::{.callout-tip icon="false" title="Approximate derivative operators by differences"}
$$ \pdv{u}{x}\approx\frac{\Delta u}{\Delta x} $$
:::

The solution $u$ is approximated by finite values $u_i$ at points $x_i$.
Taking a Taylor expansion of any function f (describing the true solution) at a point $x_0$ we obtain
$$f(x)=f(x_0)+f'(x0)(x-x_0)+f''(x_0)(x-x_0)^2/2+\ldots$$

Mostly, the function is linearized, i.e. cut after the first derivative.
The derivative is formulated from neighboring points $x_j$.
For describing the first derivative, one can use different differences:

![Forward, backward and central difference for 1D FD grid](pics/fd/finite-difference.png)

For example, the difference of $u$ between $x_2$ and $x_3$ is localized somehow between them
$$ \dv*{u}{x}_{2.5} := (u_3-u_2) / (x_3-x_2) $$

For the Poisson equation, we need a second derivative, approximated by a central difference right and left of the point, e.g., for $x_3$

$$\pdv[2]{u_3}{x}\approx
\frac{\dv*{u}{x}_{3.5}-\dv*{u}{x}_{2.5}}{(x_4-x_2)/2}
= \frac{(u_4-u_3)/(x_4-x_3)-(u_3-u_2)/(x_3-x_2)}{(x_4-x_2)/2} $$

Hence, the second derivative is expressed by the value of a point and its neighbors, represented by a difference stencil.
...
![1D Finite-Difference stencil](pics/fd/fd1dstencil.svg){#fig-1dfd-stencil}

### FDM on the general Poisson equation
Assume the Poisson equation $$-\div(a\grad u)=f$$ with a conductivity term $a$, for a 1D discretization

$$ \pdv{(a \pdv*{u}{x})}{x} = a\pdv[2]{u}{x} + \pdv{a}{x} \pdv{u}{x}  $$

It therefore contains both the second derivative (curvature) of $u$ as well as the first derivative of the conductivity $a$.
This reflects the steadiness condition of the flow $a\pdv{u}{x}$, i.e., at conductivity contrasts the potential drop rate changes.
...
If we go from 1D to 2D conditions, we have conductivities between each four nodes.
...
But how do we define the *conductivity between the two nodes* in 2D as there are two neighboring, $a_{i,j-1}$ and $a_{i,j}$. We need a mean, like $a_{i,j-1/2}=(a_{i,j-1}+a_{i,j})/2$ but instead of the arithmetic, we could also use the harmonic (mean of resistivities) or geometric (logarithmic) mean.
Furthermore, we can include the size of the cells as weighting functions.

$a_{i,j-1/2}=\frac{a_{i,j-1}\Delta y_{j-1}+a_{i,j}\Delta y_{j}}{y_j+1-y_{j-1}}$?

### 3D discretization
![3D discretization](pics/fd/fd3ddiscr.png)

![coupling coefficient ](pics/fd/fd3d_ctop.png)

## Parabolic PDEs - Time stepping

### Heat transfer in 1D with periodic boundary conditions
...
The new field is constructed by old fields only, expressed in the difference stencil (the green line represents time stepping)

![Forward (explicit) difference stencil](pics/fd/dtforward.svg)

### Implicit (Euler) method

$$ \pdv{T}{t}^n \approx \frac{T^{n+1}-T^n}{\Delta t} = a \pdv[2]{T}{z} ^{n+1} $$

Rearranging for time steps (new left and old right), this leads to

$$ T^{n+1} - \Delta t a \pdv[2]{T}{z}^{n+1} = T^n $$

Term $a \pdv[2]{T}{z}^{n+1}$ is represented by the system matrix $\vb A$ so that we can write

$$ \vb T^{n+1} - \vb A \vb T^{n+1} = (\vb I - \vb A) \vb T^{n+1} = \vb T^n $$

As $\vb A$ involves the neighbors, which are only from the new time step, this is expressed in the backward stencil difference stencil

![Backward (implicit) difference stencil](pics/fd/dtbackward.svg)

### Mixed - Crank-Nicholson method

As the finite difference in time is centered around the mean times, it seems plausible to use the temporal mean value of $T$

$$ \pdv{T}{t}^n \approx \frac{T^{n+1}-T^n}{\Delta t} = \frac12 a \pdv[2]{T}{z} ^n + \frac12 a \pdv[2]{T}{z} ^{n+1} $$

Multiplying with 2 and rearranging new and old values leads to

$$ 2 T^{n+1} - \Delta t a \pdv[2]{T}{z}^{n+1} = 2 T^n + \Delta t a \pdv[2]{T}{z}^n $$

Like for the implicit method, the operator is expressed by the system matrix $\vb A$, leading to

$$ 2\vb T^{n+1} + \vb A \vb T^{n+1} = (2\vb I + \vb A) \vb T^{n+1} = (2\vb I - \vb A) \vb T^n $$

So the new value is computed not only from its current neighbors, but also the second derivative apparent in the previous time step, expressed by the (mixed) finite difference stencil.

![Mixed (balanced) difference stencil](pics/fd/dtmixed.svg)

To summarize all three methods, the following table is created
...
$$ \vb u^{n+1} = (\frac{1}{\Delta t}\vb A + 2\vb I) \vb u^n - \vb u^{n-1} $$

![Second derivative](pics/fd/dtsecond.svg)
